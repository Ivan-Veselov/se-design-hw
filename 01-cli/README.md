...
Окружение задается двумя классамми &mdash; `SymbolsEnvironment` и `UtilitiesEnvironment`. Первый отвечает за переменные, а второй за команды-утилиты.

Интерфейс `Token` представляет из себя строку, которая может быть получена только при наличии окружения переменных.
